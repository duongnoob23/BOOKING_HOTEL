...
const HomeScreen = ({ navigation }) => {
  ...
  // Gọi các API trong try-catch
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        await Promise.all([
          dispatch(fetchServiceList()).unwrap(),
          dispatch(fetchAmenityList()).unwrap(),
          dispatch(fetchHotelList()).unwrap(),
          dispatch(fetchLocationList()).unwrap(),
        ]);
      } catch (err) {
        Alert.alert("Lỗi", err.message || "Không thể tải dữ liệu ban đầu");
      }
    };
    fetchInitialData();
  }, [dispatch]);
  ...
  const handleOpenModal = (name) => {
    ...
    try {
      ...
      Alert.alert("Lỗi", "Không thể đo vị trí modal");
    }
  };
  ...
  const handleModalCheck = (name, value) => {
    ...
    if (name === "Modal_CheckIn" || name === "Modal_CheckOut") {
      try {
        ...
        Alert.alert("Lỗi", "Không thể xử lý ngày tháng");
      }
    }
